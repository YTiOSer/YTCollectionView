...
class YTCollectionViewLayout: UICollectionViewLayout {
    ...
    /// Sections数组
    var sectionItems: [YTCollectionSectionItemType] = {
        return []
    }()
}

// MARK: 重载父类方法
extension YTCollectionViewLayout {
    
    /// 设置layout的结果和初试需要的参数等
    override func prepare() {
        super.prepare()
        
        self.register(YTCollectionReusableViewSwift.self, forDecorationViewOfKind: YTCollectionReusableViewSwift.reuseableIdentifier)
        ...
        guard let ytCollectionView = self.collectionView as? YTCollectionViewSwift, ytCollectionView.sectionItems.count > 0 else {return}
        
        let items = ytCollectionView.sectionItems
        
        for section in 0..<items.count {
            
            let sectionItem: YTCollectionSectionItemType = items.object(at: section)
            ...
            if section == 0 {
                ...
                maxHeight = self.maxHeight(with: self.sectionItemAttributesArray[section - 1], sectionItem: items.object(at: section - 1))
            }
            ...
            sectionItem.prepareLayoutWith(collectionView: self.collectionView!, topSpace: maxHeight, section: section, attributesClosure: { [weak self] (indexPath, frame) in
                ...
                guard let `self` = self else { return }
                let decorationAttributes = YTCollectionViewLayoutAttributesSwift(forDecorationViewOfKind: YTCollectionReusableViewSwift.reuseableIdentifier, with: IndexPath(row: 0, section: indexPath.section))
                decorationAttributes.frame = frame
                decorationAttributes.zIndex = -1
                decorationAttributes.sectionBgColor = sectionItem.yt_sectionBgColor
    ...
    override func layoutAttributesForDecorationView(ofKind elementKind: String, at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        if elementKind == YTCollectionReusableViewSwift.reuseableIdentifier {
...
extension YTCollectionViewLayout {
    ...
    private func maxHeight(with attributesArray: [UICollectionViewLayoutAttributes], sectionItem: YTCollectionSectionItemType?) -> CGFloat {
        ...
        if let item = sectionItem {
            longestHeight = longestHeight + item.yt_sectionInset.bottom
        }
